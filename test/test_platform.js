...
contract("Platform", function (accounts) {
    ...
    */
    ...
    it("Commence Bidding", async () => {
        ...
        // enum bidState[1]
        let bidState = await eventInstance.getEventBidState(latestEventId);
    ...
    it("Test Updating of Bid", async () => {
        ...
        await platformInstance.listEvent("Title 2", "Venue 2", 2024, 3, 11, 12, 30, 0, 1, 1, 20, accounts[1], {from: accounts[1], value: oneEth.multipliedBy(4)});
        let latestEventId = (await eventInstance.getLatestEventId()).toNumber();
        const title = await eventInstance.getEventTitle(latestEventId);
    ...
    it("Test Unsuccessful Bid Return ETH", async () => {
        // Listing of event with 1 ticket
        await platformInstance.listEvent("Title 3", "Venue 3", 2024, 3, 11, 12, 30, 0, 1, 1, 20, accounts[1], {from: accounts[1], value: oneEth.multipliedBy(4)});
        let latestEventId = (await eventInstance.getLatestEventId()).toNumber();
        const title = await eventInstance.getEventTitle(latestEventId);
    ...
    it("Test Successful Bid -> Refund Ticket -> Buy Ticket", async () => {
        // Listing of event with 1 ticket
        await platformInstance.listEvent("Title 4", "Venue 4", 2024, 3, 11, 12, 30, 0, 1, 1, 20, accounts[1], {from: accounts[1], value: oneEth.multipliedBy(4)});
        let latestEventId = (await eventInstance.getLatestEventId()).toNumber();
        const title = await eventInstance.getEventTitle(latestEventId);
