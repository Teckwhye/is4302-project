...
contract Buyer {
    ...
    function buyTickets(uint256 eventId, uint8 quantity) public isBuyer(msg.sender) {
    ...
    function refundTickets(uint256 ticketId) public isBuyer(msg.sender) {
        //include refund mechanism using tokenism
    }

    /* View available tickets for an event that are listed on the platform*/
    function viewListedEvents(uint256 eventId) public view isBuyer(msg.sender) {
        //retrieve eventid from platform & display information
    }

    /* View tickets bought */
    function viewTicketsBought(uint256 ticketId) public view isBuyer(msg.sender) {
